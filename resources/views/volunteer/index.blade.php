...
{{ trans('message.volunteer_details') }}
@endsection
    ...
    <div class="box-header">                      
      <div class="">         
        ...
        <div class="col-lg-1 pull-right">              
          <a href="{{ url('/volunteer/create') }}" class="btn btn-block btn-primary" style="text-decoration:none"><i class="fa fa-plus"></i></a>          
          <a href="{{ url('volunteer/exportcsv') }}" class="btn btn-block btn-primary" style="text-decoration:none"><i class="fa fa-file-excel-o"></i></a>        
        </div>
      </div>
    </div>
    <div class="box-body">
        ...
        <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
          <thead>
            <tr role="row">
              ...
              <th class="sorting col-lg-2" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Phone No</th> 
              <th class="sorting col-lg-2" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Email</th> 
              <th class="sorting col-lg-2" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Area of Interset</th> 
          ...
          <tbody>    
           @foreach ($volunteerslist as $volunteers)           
            <tr role="row" class="odd">
              ...
              <td>{{$volunteers->phoneno}}</td>
              <td>{{$volunteers->email}}</td>
              <td>{{$volunteers->areaofinterset}}</td>
              <td>{{$volunteers->created_at}}</td>              
              <td>
                <a href="{{ url('/volunteer/edit/'.$volunteers->id) }}" class="btn btn-primary"><i class="fa fa-edit"></i></a>
                {{Form::open(array('url' => 'volunteer/delete/' . $volunteers->id ,'method' => 'DELETE','style' => 'display:inline'))}}
                  {{Form::button('<i class="fa fa-trash"></i>', array(
                    'class' => 'btn btn-danger',
                    'data-toggle' => 'modal',
                    'data-target' => '#confirmDelete',
                    'data-title' => trans('volunteer.delete'),
